    ...
    <>
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-primary/5 via-bg to-secondary/5 py-20 md:py-28">
        ...
        <div className="pointer-events-none absolute inset-0">
          <span className="animate-float absolute left-[10%] top-[15%] text-3xl opacity-20 md:text-4xl">📚</span>
          <span className="animate-float-delay absolute right-[15%] top-[20%] text-2xl opacity-15 md:text-3xl">🤖</span>
          <span className="animate-float-slow absolute left-[20%] bottom-[20%] text-2xl opacity-15 md:text-3xl">🎓</span>
          <span className="animate-float-delay absolute right-[10%] bottom-[25%] text-3xl opacity-20 md:text-4xl">💡</span>
          <span className="animate-float absolute left-[50%] top-[10%] text-xl opacity-10 md:text-2xl">✨</span>
        </div>

        {/* Geometric dots pattern */}
        <div className="pointer-events-none absolute inset-0 opacity-[0.03]" style={{ backgroundImage: 'radial-gradient(circle, #2563EB 1px, transparent 1px)', backgroundSize: '32px 32px' }} />

        <div className="relative mx-auto max-w-6xl px-4 text-center">
          <p className="mb-4 text-sm font-semibold tracking-wide text-secondary">
            현직 에듀테크 전문가가 만드는 AI 교육 정보 플랫폼
          </p>
          <h1 className="mb-6 text-[32px] font-extrabold leading-tight text-text-primary md:text-5xl lg:text-[56px]">
            AI가 골라주는
            <br />
            <span className="bg-gradient-to-r from-primary to-secondary bg-clip-text text-transparent">나에게 딱 맞는 교육</span>
          </h1>
          ...
          <div className="mt-10 flex flex-wrap items-center justify-center gap-3 md:gap-5">
            {[
              ...
              <span key={stat.label} className="inline-flex items-center gap-1.5 rounded-full bg-white/80 px-4 py-2 text-sm font-medium text-text-secondary shadow-sm backdrop-blur-sm">
                <span>{stat.icon}</span>
          ...
          {/* Personal quote */}
          <p className="mx-auto mt-8 max-w-md text-[13px] italic text-text-secondary/60">
            &ldquo;전국 200+ 학교에 AI 교육 솔루션을 도입하며 느낀 것들을 이곳에 정리하고 있습니다.&rdquo;
          </p>
        </div>
      ...
      {/* ── Pick Tests ── 연한 블루 배경 */}
      <section className="py-16 md:py-20" style={{ backgroundColor: '#F0F7FF' }}>
        <div className="mx-auto max-w-6xl px-4">
          ...
          <div className="-mx-4 flex gap-4 overflow-x-auto px-4 pb-4 scrollbar-hide md:mx-0 md:grid md:grid-cols-3 md:gap-6 md:overflow-visible md:px-0 md:pb-0 lg:grid-cols-5">
            {PICK_TESTS.map((test) => (
              <Link
                key={test.title}
                href={test.href}
                className="group relative flex w-56 shrink-0 flex-col overflow-hidden rounded-2xl border border-border bg-surface shadow-sm transition-all duration-200 hover:-translate-y-1 hover:shadow-lg md:w-auto"
              >
                {/* Left color bar */}
                <div className="absolute left-0 top-0 h-full w-1 rounded-l-2xl" style={{ backgroundColor: test.color }} />
                {test.badge && (
        ...
        <div className="mx-auto max-w-6xl px-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {HOME_TOOLS.map((tool) => {
              ...
              return (
                <div key={tool.id} className="rounded-2xl border border-border bg-bg p-5 transition-all duration-200 hover:-translate-y-1 hover:shadow-md">
                  <div className="mb-3 flex items-center gap-2">
                    <span className="text-2xl">{tool.emoji}</span>
                    <span className="rounded-full px-2.5 py-0.5 text-[11px] font-bold" style={{ backgroundColor: priceStyle.bg, color: priceStyle.text }}>
                      {tool.pricing}
                    </span>
      ...
      <section className="py-16 md:py-20" style={{ backgroundColor: '#F8F9FA' }}>
        <div className="mx-auto max-w-6xl px-4">
          <div className="mb-6 text-center">
            <h2 className="mb-2 text-2xl font-bold text-text-primary md:text-3xl">
              📰 최신 <span className="text-primary">아티클</span>
            </h2>
      ...
      <section className="bg-gradient-to-r from-primary to-primary-dark py-16 md:py-20">
        <div className="mx-auto max-w-4xl px-4">
          <div className="mb-10 text-center">
            ...
            <p className="text-blue-100">스마트에듀픽의 다양한 콘텐츠를 만나보세요</p>
          </div>
          <div className="grid gap-4 sm:grid-cols-3">
            <Link href="/guide" className="group rounded-2xl bg-white/10 p-6 backdrop-blur-sm transition-all duration-200 hover:-translate-y-1 hover:bg-white/20">
              <span className="mb-3 block text-3xl">📖</span>
              <h3 className="mb-1 text-base font-bold text-white group-hover:text-accent">AI 활용 가이드</h3>
              <p className="text-sm text-blue-100">입문자·교사·학부모를 위한 단계별 가이드</p>
            </Link>
            <Link href="/test/ai-literacy" className="group rounded-2xl bg-white/10 p-6 backdrop-blur-sm transition-all duration-200 hover:-translate-y-1 hover:bg-white/20">
              <span className="mb-3 block text-3xl">🎯</span>
              <h3 className="mb-1 text-base font-bold text-white group-hover:text-accent">AI 활용 능력 진단</h3>
              <p className="text-sm text-blue-100">나의 AI 레벨을 확인하고 맞춤 학습 경로를 받으세요</p>
            </Link>
            <Link href="/test/career-ai" className="group rounded-2xl bg-white/10 p-6 backdrop-blur-sm transition-all duration-200 hover:-translate-y-1 hover:bg-white/20">
              <span className="mb-3 block text-3xl">💼</span>
              <h3 className="mb-1 text-base font-bold text-white group-hover:text-accent">AI 시대 직업 적성</h3>
              <p className="text-sm text-blue-100">AI 대체 안전도와 커리어 방향을 확인해보세요</p>
            </Link>
          </div>
        </div>
      </section>
      ...
      <section className="py-16 md:py-20" style={{ backgroundColor: '#1E293B' }}>
        <div className="mx-auto max-w-4xl px-4 text-center">
          <h2 className="mb-4 text-2xl font-bold text-white md:text-3xl">
            현직 에듀테크 <span className="text-accent">전문가</span>가 만듭니다
          </h2>
          ...
          <div className="grid gap-6 sm:grid-cols-3">
            <div className="rounded-2xl border border-slate-600 bg-slate-800/50 p-6">
              ...
              <p className="text-sm font-medium text-white">학교 AI 교육 지원</p>
              <p className="mt-1 text-xs text-slate-400">전국 200개+ 학교에 AI 교육 솔루션 도입 지원</p>
            </div>
            <div className="rounded-2xl border border-slate-600 bg-slate-800/50 p-6">
              <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-gradient-to-br from-secondary/20 to-accent/20">
                <span className="text-3xl">🧠</span>
              </div>
              <h3 className="mb-2 text-xl font-bold text-secondary">3년+</h3>
              <p className="text-sm font-medium text-white">AI 코스웨어 개발</p>
              <p className="mt-1 text-xs text-slate-400">맞춤학습 플랫폼 기획·개발 3년+ 경험</p>
            </div>
            <div className="rounded-2xl border border-slate-600 bg-slate-800/50 p-6">
              <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-gradient-to-br from-accent/20 to-primary/20">
                <span className="text-3xl">📝</span>
              </div>
              <h3 className="mb-2 text-xl font-bold text-accent">멀티채널</h3>
              <p className="text-sm font-medium text-white">콘텐츠 운영</p>
              <p className="mt-1 text-xs text-slate-400">블로그 · 유튜브에서 AI 교육 콘텐츠 제작</p>
            </div>
          </div>
        </div>
      </section>
