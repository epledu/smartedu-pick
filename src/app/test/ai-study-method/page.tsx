...
import { studyMethodQuestions } from '@/data/tests/ai-study-method';
import type { StudyMethodType } from '@/data/tests/ai-study-method';
import { calculateStudyMethod } from '@/lib/test-utils';
import { event as gaEvent } from '@/lib/analytics';
import TestProgress from '@/components/test/TestProgress';

type Phase = 'intro' | 'questions' | 'calculating';

export default function AIStudyMethodTestPage() {
  const router = useRouter();
  const [phase, setPhase] = useState<Phase>('intro');
  const [currentIndex, setCurrentIndex] = useState(0);
  const [answers, setAnswers] = useState<StudyMethodType[]>([]);
  const [isTransitioning, setIsTransitioning] = useState(false);

  const handleStart = () => {
    gaEvent('test_start', { test_name: 'ai-study-method' });
    setPhase('questions');
  };
  ...
  const handleAnswer = useCallback(
    (type: StudyMethodType) => {
      ...
      if (currentIndex < studyMethodQuestions.length - 1) {
        ...
        const result = calculateStudyMethod(newAnswers);
        gaEvent('test_complete', {
          test_name: 'ai-study-method',
          result_type: result.primary,
        });
        setTimeout(() => {
          router.push(
            `/test/ai-study-method/result?primary=${result.primary}&secondary=${result.secondary}`
          );
        }, 1200);
      }
    },
      ...
      <div className="flex min-h-[70vh] items-center py-16 md:py-24">
        <div className="mx-auto max-w-lg px-4 text-center">
          ...
          <p className="mb-2 text-base text-text-secondary md:text-lg">
            AI 도구 + 학습법을 조합한 나만의 학습 루틴을 찾아보세요
          </p>
          <p className="mb-8 text-sm text-text-secondary">
            8가지 AI 공부법 중 나에게 딱 맞는 조합은?
          </p>
  ...
  // Question Screen
  const question = studyMethodQuestions[currentIndex];
    ...
    <div className="min-h-[70vh] py-8 md:py-16">
      <div className="mx-auto max-w-xl px-4">
        ...
        <TestProgress
          ...
          total={studyMethodQuestions.length}
        />
